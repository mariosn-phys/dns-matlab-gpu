...
for l = 1:4
        Y = cumsum([B(l,:); l*Y(1:N-3,:).*X]);    % Recursion for diagonals
        D = l*Z.*(C.*repmat(diag(D),1,N-2) - D);  % Recursion for off-diagonals
     D(L) = Y(N-2,:);                             % Correct the main diagonal
DM(:,:,l) = D;                                    % Store the current D in DM
end
